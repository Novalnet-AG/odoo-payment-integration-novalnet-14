...
class AcquirerNovalnet(models.Model):
    ...
    novalnet_callbackscript_debug_mode = fields.Boolean(_('Enable Debug Mode'), help=_('Set the debug mode to execute the merchant script in debug mode'))
    novalnet_callbackscript_test_mode = fields.Boolean(_('Enable Test Mode'))
    novalnet_callbackscript_email = fields.Boolean(_('Enable E-mail notification for callback'))
    ...
    def _generate_encode(self, _value, _access_key ):
        _value = str(_value)
        _crc = '%u' % (binascii.crc32(_value) & 0xffffffff)
        _data = _crc + "|" + _value
        _data = binascii.hexlify(_data+_access_key)
        _encoded = base64.b64encode(_data)
        return _encoded[::-1]
    ...
    def novalnet_form_generate_values(self, _values):
        ...
        _remote_address = request.httprequest.headers.environ.get("REMOTE_ADDR")
        ...
        _transaction_values = {
            ...
            'system_name': 'odoo10',
            'system_version': '10.0-NN2.0.0',
            'system_ip': '127.0.0.1' if socket.inet_aton(_server_address) else _server_address,
            'system_url': '%s' % urlparse.urljoin(_base_url, ''),
            ...
            'remote_ip': '127.0.0.1' if socket.inet_aton(_remote_address) else _remote_address,
            'return_url': '%s' % urlparse.urljoin(_base_url, '/payment/novalnet/return/'),
            'error_return_url': '%s' % urlparse.urljoin(_base_url, '/payment/novalnet/return/'),
            'return_method': 'POST',
    ...
    def _parameters_validation(self, _transaction_values):
        ...
        _transaction_values['product']   = self._generate_encode(_product, _access_key)
        _transaction_values['tariff']    = self._generate_encode(_tariff, _access_key)
        _transaction_values['auth_code'] = self._generate_encode(_auth_code, _access_key)
        _transaction_values['amount']    = self._generate_encode(_transaction_values['amount'], _access_key)
        _transaction_values['test_mode'] = self._generate_encode(_transaction_values['test_mode'], _access_key)
        _transaction_values['uniqid']    = self._generate_encode(time.time(), _access_key)
        """Forming hash value """
        _transaction_values['hash']      = self._generate_hash(_transaction_values, _access_key)
        return _transaction_values
...
class TxNovalnet(models.Model):
    ...
    def _novalnet_form_validate(self, data):
        ...
        _url = '%s' % urlparse.urljoin(_base_url, '')
    ...
    def _form_bank_comments(self, _payment_name, _bank_details, _payment_acquirer, _order_no, _test_mode,_payment_method = ''):
        ...
        _due_date = datetime.strptime(_bank_details['due_date'], '%Y-%m-%d').strftime(DEFAULT_SERVER_DATE_FORMAT)
    ...
    def _decode(self, _data, _access_key):
        try:
            ...
            if (_crc != '%u' % (binascii.crc32(_value) & 0xffffffff)):
