...
class AcquirerNovalnet(models.Model):
    ...
    provider = fields.Selection(selection_add=[('novalnet', 'Novalnet')], ondelete={'novalnet': 'set default'})
    ...
    novalnet_completion_order_status = fields.Selection([('draft', _('Draft')), ('pending', _('Pending')),
                                                 ...
                                                 ('cancel', _('Canceled'))
                                                 ],_('Completed order status'), default='done')
    ...
    novalnet_pending_order_status = fields.Selection([('draft', _('Draft')), ('pending', _('Pending')),
                                                 ('done', _('Done')), ('error', _('Error')),
                                                 ('cancel', _('Canceled'))
                                                 ],_('Payment pending order status'), default='pending')
    ...
    novalnet_on_hold_cancellation_status = fields.Selection([('draft', _('Draft')), ('pending', _('Pending')),
                                                 ('done', _('Done')), ('error', _('Error')),
                                                 ('cancel', _('Canceled'))
                                                 ],_('Canceled order status'),default='cancel')
    novalnet_manual_check_limit = fields.Selection([('capture',_('Capture')),('authorize',_('Authorize'))],_('Payment Action'), default='capture', help=_('Choose whether or not the payment should be charged immediately. Capture completes the transaction by transferring the funds from buyer account to merchant account. Authorize verifies payment details and reserves funds to capture it later, giving time for the merchant to decide on the order'))
    novalnet_manual_check_limit_amount = fields.Char(_('Minimum transaction amount for authorization (in minimum unit of currency. E.g. enter 100 which is equal to 1.00)'), help=_('In case the order amount exceeds the mentioned limit, the transaction will be set on-hold till your confirmation of the transaction. You can leave the field empty if you wish to process all the transactions as on-hold.'))
    novalnet_sepa_due_date = fields.Char(_('Payment due date (in days)'), help=_('Enter the number of days after which the payment should be processed (must be between 2 and 14 days)'))
    ...
    novalnet_callbackscript_test_mode = fields.Boolean(_('Allow manual testing of the Notification / Webhook URL'),default=False,help=_('This option will allow performing a manual execution. Please disable this option before setting your shop to LIVE mode, to avoid unauthorized calls from external parties (excl. Novalnet).'))
    novalnet_callbackscript_email_to = fields.Char(_('Send e-mail to'), help=_('E-mail address of the recipient'))
    ...
    def novalnet_form_generate_values(self, _values):
        ...
        _transaction_values = {
            ...
            'system_version'     : __version__+'-NN2.3.0',
            ...
            'implementation'     : 'ENC',
            'rrt'                : 1,
            'customer_no'        :  _values.get('billing_partner_id') if _values.get('billing_partner_id') != '' else values.get('partner_id'),
        }
        ...
        _validate_data = {
            ...
            'last_name'        : _values.get('billing_partner_last_name') if _values.get('billing_partner_last_name') != '' else _values.get('billing_partner_first_name'),
            'tel'              : _values.get('billing_partner_phone'),
        }
        
        for key in _validate_data:
            if _validate_data[key] != False:
               _transaction_values[key] = _validate_data[key]
               if key == 'street':
                  _transaction_values['search_in_street'] = 1
        ...
        """ Check for company """
        if _values.get('billing_partner_commercial_company_name') != False:
            _transaction_values['company'] = _values.get('billing_partner_commercial_company_name')
    ...
    def _parameters_validation(self, _transaction_values):
        ...
        if _invoice_due_date  and _invoice_due_date >=7:
            _transaction_values['invoice_due_date'] = _invoice_due_date
        
        if _cp_slip_expiry_date:
            _transaction_values['cashpayment_slip_expiry_date'] = _cp_slip_expiry_date
...
class TxNovalnet(models.Model):
    ...
    def _handle_s2s_response(self, data):
        ...
        if 'tid' in data and data.get('tid') != '':
            ...
            elif _payment_key == '40':
               _payment_type = 'GUARANTEED_DIRECT_DEBIT_SEPA'
    ...
    def _novalnet_form_validate(self, data):
        ...
        if 'tid' in data and data.get('tid') != '':
            ...
            if _status_code in  [100, 90]:
                ...
                """ Pending process """
                if _tid_status_code in [86, 90, 83, 75] and data['transaction']['payment_type'] in ['PRZELEWY24', 'PAYPAL', 'POSTFINANCE', 'POSTFINANCE_CARD']:
                    _callback_amount = 0
                    _order_status = _payment_acquirer.novalnet_pending_order_status
                    
                """ Pending process """
                if _tid_status_code == 75 and data['transaction']['payment_type'] in ['GUARANTEED_INVOICE', 'INSTALMENT_INVOICE', 'GUARANTEED_DIRECT_DEBIT_SEPA', 'INSTALMENT_DIRECT_DEBIT_SEPA']:
                    _order_status = _payment_acquirer.novalnet_pending_order_status
                ...
                """ Check for Multibanco payment """
                if data['payment_type'] == 'MULTIBANCO' and _tid_status_code == 100:
                    _callback_amount = 0
                    _comments = _comments + self._multibanco_ref_comments(data['transaction'])
                ...
                if data['payment_type'] in ['INVOICE_START', 'GUARANTEED_INVOICE', 'INSTALMENT_INVOICE'] and _tid_status_code in [100, 91]:
                    if data['payment_type'] == 'INVOICE_START':
                ...
                if data['payment_type'] in ['CREDITCARD', 'DIRECT_DEBIT_SEPA', 'GUARANTEED_DIRECT_DEBIT_SEPA'] and _tid_status_code in [99, 98, 100] and self.type == 'form_save':
                    ...
                    if 'transaction' in data:
                        if data['payment_type'] in ['CREDITCARD']: 
                            ...
                            token_name   = (_('IBAN %s') % (data['transaction']['payment_data']['iban']))
                        if token_name != '':
                            token_data = {
    ...
    def _get_payment_method(self, _key):
        _payment_array = {
            '6'  : { 'payment_name': _('Credit/Debit Cards'), 'payment_method_type': 'novalnet_cc' },
            ...
            '59' : { 'payment_name': _('Barzahlen/viacash'), 'payment_method_type': 'novalnet_cashpayment' },
            ...
            '73' : { 'payment_name': _('Multibanco'), 'payment_method_type': 'novalnet_multibanco' },
            '14' : { 'payment_name': _('Direct Carrier Billing'), 'payment_method_type': 'novalnet_carrier_billing' },
            '87' : { 'payment_name': _('PostFinance Card'), 'payment_method_type': 'novalnet_postfinance_card' },
            '88' : { 'payment_name': _('PostFinance E-Finance'), 'payment_method_type': 'novalnet_postfinance' },
            '44' : { 'payment_name': _('Bancontact'), 'payment_method_type': 'novalnet_bancontact' },
            '96' : { 'payment_name': _('Instalment by Invoice'), 'payment_method_type': 'novalnet_instalment_invoice' },
            '97' : { 'payment_name': _('Instalment by Direct Debit SEPA'), 'payment_method_type': 'novalnet_instalment_sepa' },
            'CREDITCARD'  : { 'payment_name': _('Credit/Debit Cards'), 'payment_method_type': 'novalnet_cc' },
            ...
            'CASHPAYMENT' : { 'payment_name': _('Barzahlen/viacash'), 'payment_method_type': 'novalnet_cashpayment' },
            ...
            'MULTIBANCO' : { 'payment_name': _('Multibanco'), 'payment_method_type': 'novalnet_multibanco' },
            'CARRIER_BILLING' : { 'payment_name': _('Direct Carrier Billing'), 'payment_method_type': 'novalnet_carrier_billing' },
            'POSTFINANCE_CARD' : { 'payment_name': _('PostFinance Card'), 'payment_method_type': 'novalnet_postfinance_card' },
            'POSTFINANCE' : { 'payment_name': _('PostFinance E-Finance'), 'payment_method_type': 'novalnet_postfinance' },
            'BANCONTACT' : { 'payment_name': _('Bancontact'), 'payment_method_type': 'novalnet_bancontact' },
            'INSTALMENT_INVOICE' : { 'payment_name': _('Instalment by Invoice'), 'payment_method_type': 'novalnet_instalment_invoice' },
            'INSTALMENT_DIRECT_DEBIT_SEPA' : { 'payment_name': _('Instalment by Direct Debit SEPA'), 'payment_method_type': 'novalnet_instalment_sepa' },
        }
    ...
    def _form_basic_comments(self, _payment_name, _tid, _payment_type, _test_mode, _tid_status):
        _comments = ''
        _comments =  _comments +_payment_name + '\n' + _('Novalnet transaction ID: %s')% _tid + '\n' + _('Test order') + '\n' if _test_mode == '1' else '\n' + _payment_name + '\n' + _('Novalnet transaction ID: %s')% _tid + '\n'
        
        if _payment_type in ['GUARANTEED_INVOICE','GUARANTEED_DIRECT_DEBIT_SEPA']:
           _comments = _comments + _('This is processed as a guarantee payment') + '\n'
        if _tid_status == 75:
            if _payment_type in ['GUARANTEED_DIRECT_DEBIT_SEPA', '40']:
                _comments = _comments + '\n' + _('Your order is under verification and we will soon update you with the order status. Please note that this may take upto 24 hours.')
            if _payment_type in ['GUARANTEED_INVOICE', '41']:
                _comments = _comments + '\n' + _('Your order is under verification and once confirmed, we will send you our bank details to where the order amount should be transferred. Please note that this may take upto 24 hours.')
        return _comments
    ...
    def _form_bank_comments(self, _invoice_details, _payment_acquirer, _order_no,_callback = False):
        _tid_status = int( _invoice_details['tid_status'] )
        _amount = float( _invoice_details['amount'])/100.0
        _amount_formated = ('%.2f' % _amount)
        _comments = '\n' + _('Please transfer the amount of ') + _amount_formated +' '+ _invoice_details['currency'] +_(' to the following account ') 
        if _tid_status == 100:
           _due_date = datetime.strptime(_invoice_details['due_date'], '%Y-%m-%d').strftime('%d.%m.%Y')
           _comments = _comments + _('on or before ')+ _due_date
        _comments = _comments + '\n\n' +_('Account holder: ') + _invoice_details['invoice_account_holder'] + '\n' +_('Bank: ') + _invoice_details['invoice_bankname'] + '\n' +_('Place: ') + _invoice_details['invoice_bankplace'] + '\n' +_('IBAN: ') + _invoice_details.get("invoice_iban") + '\n' +_('BIC: ') + _invoice_details["invoice_bic"]  + '\n\n' + _('Please use any of the following payment references when transferring the amount. This is necessary to match it with your corresponding order') + '\n' + _('Payment Reference 1: ') + 'BNR-' + str(_payment_acquirer.novalnet_product.strip()) + '-' + _order_no + '\n' + _('Payment Reference 2: ') + ' TID ' + str(_invoice_details['tid'])
        return _comments
        
    """ Generate Multibanco comments
    >>> _multibanco_ref_comments(self, _response_data)
    string
    """
    def _multibanco_ref_comments(self, _response_data):
        _amount = float( _response_data['amount'])/100.0
        _amount_formated = ('%.2f' % _amount)
        _comments = '\n'+ _('Please use the following payment reference details to pay the amount of %s', _amount_formated) + _(' %s at a Multibanco ATM or through your internet banking.', _response_data['currency'])
        _comments = _comments + '\n'+_('Partner Payment Reference: %s') % _response_data['partner_payment_reference']
        return _comments
    ...
    def novalnet_s2s_do_transaction(self, **kwargs):
        ...
        if _transaction_values['payment_type'] == 'GUARANTEED_DIRECT_DEBIT_SEPA':
           _transaction_values['payment_type'] = 'DIRECT_DEBIT_SEPA'
