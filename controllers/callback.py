...
class NovalnetCallback(http.Controller):
    ...
    def _update_initial_payment(self, _cr_data):
        ...
        if self._get_request['tid_status'] in self._success_code[self._get_request['payment_type']]:
            ...
            """ Assign order status """
            if _payment_key == '49':
                _order_status = self._novalnet_configuration.novalnet_ideal_order_completion_status
            if _payment_key == '50':
                _order_status = self._novalnet_configuration.novalnet_eps_order_completion_status
            if _payment_key == '33':
                _order_status = self._novalnet_configuration.novalnet_sofort_order_completion_status
            if _payment_key == '69':
                _order_status = self._novalnet_configuration.novalnet_giropay_order_completion_status
            ...
            if _payment_key == '27':
                ...
                _callback_amount = 0
                _order_status = self._novalnet_configuration.novalnet_invoice_order_completion_status
            ...
            elif _payment_key == '34' and self._get_request['tid_status'] in ['90','85'] :
                """ Process pending payment """
                _callback_amount = 0
    ...
    def _zero_level_process(self,_cr_data):
        if 'subs_billing' in self._get_request and self._get_request['subs_billing'] == '1':
            ...
            """ Update callback comments and order status """
            _cr_data.execute('UPDATE payment_transaction SET state=%s, state_message=CONCAT(state_message, %s) WHERE reference=%s',(self._novalnet_configuration.novalnet_przelewy24_order_completion_status, _comments, self._get_order['order_no']))
            ...
            if self._novalnet_configuration.novalnet_przelewy24_order_completion_status == 'done':
            ...
            if self._get_request['tid_status'] =='100' and self._get_request['status'] == '100':
                ...
                if self._get_request['tid_status'] in ['91','99'] and self._get_order['gateway_status'] == 75:
                   ...
                   _comments=  '\n\nNovalnet callback received. The transaction status has been changed from pending to on hold for the TID: %s on %s.' % (self._get_request['shop_tid'], datetime.datetime.now().strftime(DEFAULT_SERVER_DATETIME_FORMAT)) + _comments 
                    ...
                    if self._get_request['payment_type'] in ['GUARANTEED_INVOICE','INVOICE_START'] and self._get_request['tid_status'] == '100' and  self._get_order['gateway_status'] == 91:
                        _order_status = self._novalnet_configuration.novalnet_invoice_order_completion_status
                    elif self._get_request['payment_type'] in ['GUARANTEED_DIRECT_DEBIT_SEPA','DIRECT_DEBIT_SEPA'] and self._get_request['tid_status'] == '100' and self._get_order['gateway_status'] == 99:
                        _order_status = self._novalnet_configuration.novalnet_sepa_order_completion_status
                        
                    elif self._get_request['payment_type'] == 'CREDITCARD' and self._get_request['tid_status'] == '100' and self._get_order['gateway_status'] == 98:
                        _order_status = self._novalnet_configuration.novalnet_cc_order_completion_status
                        
                    elif self._get_request['payment_type'] == 'PAYPAL' and self._get_request['tid_status'] == '100' and self._get_order['gateway_status'] in [85,90]:
                        _order_status = self._novalnet_configuration.novalnet_paypal_order_completion_status
                ...
                _cr_data.commit()
                _orginal_order_no = self._get_order['order_no'].split('-')
                if self._novalnet_configuration.novalnet_invoice_order_completion_status == 'done' or self._novalnet_configuration.novalnet_cashpayment_order_completion_status == 'done' or self._novalnet_configuration.novalnet_prepayment_order_completion_status == 'done' or self._novalnet_configuration.novalnet_sepa_order_completion_status == 'done' or self._novalnet_configuration.novalnet_cc_order_completion_status == 'done' or self._novalnet_configuration.novalnet_eps_order_completion_status == 'done' or self._novalnet_configuration.novalnet_ideal_order_completion_status == 'done' or self._novalnet_configuration.novalnet_giropay_order_completion_status == 'done' or self._novalnet_configuration.novalnet_sofort_order_completion_status == 'done':
                ...
                if self._novalnet_configuration.novalnet_invoice_order_completion_status == 'done' or self._novalnet_configuration.novalnet_cashpayment_order_completion_status == 'done' or self._novalnet_configuration.novalnet_prepayment_order_completion_status == 'done' or self._novalnet_configuration.novalnet_sepa_order_completion_status == 'done' or self._novalnet_configuration.novalnet_cc_order_completion_status == 'done' or self._novalnet_configuration.novalnet_eps_order_completion_status == 'done' or self._novalnet_configuration.novalnet_ideal_order_completion_status == 'done' or self._novalnet_configuration.novalnet_giropay_order_completion_status == 'done' or self._novalnet_configuration.novalnet_sofort_order_completion_status == 'done' == 'done':
    ...
    def _second_level_process(self, _cr_data):
        ...
        if self._is_valid_digit(self._get_order['callback_amount']) < self._is_valid_digit(self._get_order['total_amount']):
            ...
            if _paid_amount >= self._is_valid_digit(self._get_order['total_amount']) and self._get_request['payment_type'] in ['INVOICE_CREDIT','CASHPAYMENT_CREDIT']:
                ...
                elif self._get_request['payment_type'] == 'CASHPAYMENT_CREDIT':
                    _callback_status = self._novalnet_configuration.novalnet_cashpayment_callback_order_status                
    ...
    def _check_authentication(self):
        ...
        remote_address = self.get_client_ip()
