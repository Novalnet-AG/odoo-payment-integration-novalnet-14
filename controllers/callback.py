...
class NovalnetCallback(http.Controller):
    ...
    _zero_level_payment_type = { 'CREDITCARD', 'INVOICE_START', 'DIRECT_DEBIT_SEPA', 'GUARANTEED_INVOICE', 'GUARANTEED_DIRECT_DEBIT_SEPA', 'PAYPAL', 'ONLINE_TRANSFER', 'IDEAL', 'GIROPAY', 'PRZELEWY24', 'EPS', 'CASHPAYMENT', 'MULTIBANCO', 'CARRIER_BILLING', 'POSTFINANCE', 'POSTFINANCE_CARD', 'BANCONTACT', 'INSTALMENT_INVOICE', 'INSTALMENT_DIRECT_DEBIT_SEPA'}

    """ First level payment types """
    _first_level_payment_type = {'PRZELEWY24_REFUND', 'RETURN_DEBIT_SEPA', 'REVERSAL', 'CREDITCARD_BOOKBACK', 'CREDITCARD_CHARGEBACK', 'PAYPAL_BOOKBACK', 'REFUND_BY_BANK_TRANSFER_EU','GUARANTEED_INVOICE_BOOKBACK','GUARANTEED_SEPA_BOOKBACK', 'CASHPAYMENT_REFUND', 'CARRIER_BILLING_REFUND', 'POSTFINANCE_REFUND', 'INSTALMENT_INVOICE_BOOKBACK', 'INSTALMENT_SEPA_BOOKBACK'}

    """ Second level payment types """
    _second_level_payment_type = { 'ONLINE_TRANSFER_CREDIT','INVOICE_CREDIT', 'CREDIT_ENTRY_CREDITCARD', 'CREDIT_ENTRY_SEPA', 'DEBT_COLLECTION_SEPA', 'DEBT_COLLECTION_CREDITCARD', 'CASHPAYMENT_CREDIT', 'CREDIT_ENTRY_DE', 'DEBT_COLLECTION_DE', 'MULTIBANCO_CREDIT'}
    ...
    _payment_groups = {
        ...
        'novalnet_carrier_billing' : {
            'CARRIER_BILLING',
            'CARRIER_BILLING_REFUND'
        },
        'novalnet_postfinance_card' : {
            'POSTFINANCE_CARD',
            'POSTFINANCE_REFUND'
        },
        'novalnet_postfinance' : {
            'POSTFINANCE',
            'POSTFINANCE_REFUND'
        },
        'novalnet_bancontact' : {
    ...
    def _get_order_reference(self, _cr_data):
        ...
        if not _fetch_order_details:
            if (self._get_request['payment_type'] in self._zero_level_payment_type or self._get_request['payment_type'] == 'ONLINE_TRANSFER_CREDIT'):
                ...
                _comments = self._update_initial_payment(_cr_data)
                if self._get_request['payment_type'] == 'ONLINE_TRANSFER_CREDIT':
                   """ Get order details from the novalnet_callback table using TID """
                   order_details = _cr_data.execute('SELECT order_no, total_amount, callback_amount, payment_method_type,gateway_status,additional_info from novalnet_callback WHERE reference_tid=%s', (self._get_request['shop_tid'],))
                   _fetch_order_details = _cr_data.dictfetchone()
                else:
    ...
    def _update_initial_payment(self, _cr_data):
        ...
        if 'payment_id' in self._get_request and self._get_request['payment_id'] != '':
           _payment_key = self._get_request['payment_id']
        elif 'key' in self._get_request and self._get_request['key'] != '':
             _payment_key = self._get_request['key']
        ...
        if self._get_request['payment_type'] in self._success_code and self._get_request['tid_status'] in self._success_code[self._get_request['payment_type']]:

            """ Assign order status """
            if self._get_request['tid_status'] == '100':
               _order_status = self._novalnet_configuration.novalnet_completion_order_status
                
            _callback_amount = self._get_request['amount']
            
            if _payment_key in ['34','78', '87', '88'] and self._get_request['tid_status'] in ['90','85','86', '83'] :
                """ Process pending payment """
                _callback_amount = 0
                _order_status = self._novalnet_configuration.novalnet_pending_order_status
                
            """ Pending process """
            if self._get_request['tid_status'] == '75' and _payment_key in ['41', '40', '96', '97']:
                _order_status = self._novalnet_configuration.novalnet_pending_order_status

            """ Process invoice, guaranteed and instalment payments """
            if _payment_key in ['27', '73', '96', '97', '41', '40'] and self._get_request['tid_status'] in ['100', '91', '99']:
                _callback_amount = 0
                _order_status = self._novalnet_configuration.novalnet_completion_order_status
                ...
                if  _payment_key == '73':
                    _comments = _comments + request.env['payment.transaction'].sudo()._multibanco_ref_comments(self._get_request)
            ...
            _cr_data.execute('INSERT INTO novalnet_callback (order_no, callback_amount, total_amount, reference_tid, callback_tid, callback_log, payment_method_type, gateway_status) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)', (self._get_request['order_no'], _callback_amount, self._get_request['amount'], self._get_request['shop_tid'], self._get_request['shop_tid'], _url, _payment_property['payment_method_type'], self._get_request['tid_status']))
            _cr_data.commit()
    ...
    def _zero_level_process(self,_cr_data):
        if 'subs_billing' in self._get_request and self._get_request['subs_billing'] == '1':
        ...
        elif self._get_request['payment_type'] in ['PRZELEWY24', 'POSTFINANCE', 'POSTFINANCE_CARD'] and self._get_order['gateway_status'] in [86, 83]:
            _callback_paid_amount = float(self._get_request['amount'])/100.0
        ...
        elif self._get_request['payment_type'] in ['GUARANTEED_INVOICE','GUARANTEED_DIRECT_DEBIT_SEPA','INVOICE_START','DIRECT_DEBIT_SEPA','CREDITCARD','PAYPAL', 'INSTALMENT_INVOICE','INSTALMENT_DIRECT_DEBIT_SEPA'] and self._get_order['gateway_status'] in [75,91,99,98,85,90]:
            if self._get_request['status'] == '100':
                ...
                if self._get_request['payment_type'] in ['GUARANTEED_INVOICE','GUARANTEED_DIRECT_DEBIT_SEPA','INVOICE_START','INSTALMENT_INVOICE','INSTALMENT_DIRECT_DEBIT_SEPA']:
                    _payment_property = request.env['payment.transaction'].sudo()._get_payment_method(_get_payment_key)
                ...
                if self._get_request['tid_status'] in [ '91', '99' ] and self._get_order['gateway_status'] == 75:
                   _order_status = self._novalnet_configuration.novalnet_on_hold_confirmation_status
                   _comments=  '\n\nNovalnet callback received. The transaction status has been changed from pending to on hold for the TID: %s on %s.' % (self._get_request['shop_tid'], datetime.datetime.now().strftime(DEFAULT_SERVER_DATETIME_FORMAT)) 
                   if self._get_request['tid_status'] == '91':
                      _comments = _comments + '\n' + request.env['payment.transaction'].sudo()._form_bank_comments(self._get_request, self._novalnet_configuration,self._get_request['order_no'],True)    
                    ...
                    if self._get_request['payment_type'] in ['GUARANTEED_INVOICE','INVOICE_START', 'INSTALMENT_INVOICE'] and self._get_order['gateway_status'] in [75,91]:
    ...
    def _second_level_process(self, _cr_data):
        ...
        if self._is_valid_digit(self._get_order['callback_amount']) < self._is_valid_digit(self._get_order['total_amount']):
            ...
            if _paid_amount >= self._is_valid_digit(self._get_order['total_amount']) and self._get_request['payment_type'] in ['INVOICE_CREDIT','CASHPAYMENT_CREDIT', 'MULTIBANCO_CREDIT']:
    ...
    def _send_novalnet_callback_email(self, _message,_request = '', _order_no= ''):
        ...
        if self._novalnet_configuration.novalnet_callbackscript_email_to and self._novalnet_configuration.novalnet_callbackscript_email_to != '' :                       
            ...
            if _request['payment_id'] in ['40','41']:
                ...
                _body = _message
            msg = MIMEMultipart()
            msg['From'] = 'OdooBot <odoobot@example.com>'
    ...
    def _get_payment_keys(self,_payment_name):
        _payment_key_array = {
            ...
            'novalnet_carrier_billing' : '14',
            'novalnet_postfinance' : '88',
